...
class URLSessionApiSrevices: GetQuestionsProtocol {
    ...
    func getQuestions(tag:String, completion: @escaping (_ responce:Questionanswer<ItemModel>) -> ()) {
        ...
        getDataFromCacheByTimer(url: (self.dataTask?.currentRequest?.url)!) { (items) in
            self.dataTask?.cancel()
    ...
    func convertDataToModel(responceData:Data?) -> [ItemModel]? {
        ...
        guard let responceData = responceData else { self.error = NSLocalizedString("data loading error", comment: "data from cashe loading error"); return nil}
